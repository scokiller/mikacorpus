'use strict';
(()=>{
  const ENDPOINT='https://xifxrkjvsrzexmuqtsvw.supabase.co/functions/v1/mikavideo-private/module?name=mikavideo-v41';
  ...
  async function boot(){
    try{
      ...
      catch(e){throw new Error(`fetch Safe Enhancement v4.1: ${describeError(e)}`)}
      ...
      if(!expected)throw new Error('header SHA Safe Enhancement absent');
      if(actual!==expected)throw new Error(`SHA-256 Safe Enhancement refusé · attendu ${expected.slice(0,12)} · reçu ${actual.slice(0,12)}`);
      const src=new TextDecoder().decode(bytes);
      try{(new Function(src))()}catch(e){throw new Error(`exécution module: ${describeError(e)}`)}
      try{log(`Mika Safe Enhancement v4.1 privé chargé · r${r.headers.get('x-mikavideo-revision')||'?'} · SHA ✓`)}catch{}
    }catch(e){try{log(`Mika Safe Enhancement v4.1 non chargé: ${describeError(e)}`)}catch{}}
  }
