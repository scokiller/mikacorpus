'use strict';
(()=>{
  const ENDPOINT='https://xifxrkjvsrzexmuqtsvw.supabase.co/functions/v1/mikavideo-private/module?name=mikavideo-v07';
  ...
  async function boot(){
    try{
      ...
      catch(e){throw new Error(`fetch Reality Lab: ${describeError(e)}`)}
      ...
      if(!expected)throw new Error('header SHA Reality Lab absent');
      if(actual!==expected)throw new Error(`SHA-256 Reality Lab refusé · attendu ${expected.slice(0,12)} · reçu ${actual.slice(0,12)}`);
      const src=new TextDecoder().decode(bytes);
      try{(new Function(src))()}catch(e){throw new Error(`exécution module: ${describeError(e)}`)}
      try{log(`Mika Reality Lab v0.7 privé chargé · r${r.headers.get('x-mikavideo-revision')||'?'} · SHA ✓`)}catch{}
    }catch(e){try{log(`Mika Reality Lab non chargé: ${describeError(e)}`)}catch{}}
  }
